...
```{r}
mr_trash_wheel_df = 
  read_excel("data/202309 Trash Wheel Collection Data.xlsx", sheet = "Mr. Trash Wheel", range = "A2:M550") |> 
...
```{r}
prof_trash_wheel_df = 
  read_excel("data/202309 Trash Wheel Collection Data.xlsx", sheet = "Professor Trash Wheel", range = "A2:L97") |> 
  janitor::clean_names() |> # use reasonable variable names
  drop_na(dumpster) |> # omit rows that do not include dumpster-specific data
  mutate(homes_powered = weight_tons * 500 / 30,
         name = "Professor")
...
gwy_trash_wheel_df = 
  read_excel("data/202309 Trash Wheel Collection Data.xlsx", sheet = "Gwynnda Trash Wheel", range = "A2:J110") |> 
  janitor::clean_names() |> # use reasonable variable names
  drop_na(dumpster) |> # omit rows that do not include dumpster-specific data
  mutate(homes_powered = weight_tons * 500 / 30,
         name = "Gwynnda")
...
Then, `r length(unique(all_participants_df$study_id[is.na(all_participants_df$id)]))` participants are only in the amyloid datset, `r sum(is.na(all_participants_df$study_id))` participants are only in the baseline datset.  
...
The result contains `r nrow(both_participants_df)` observations of `r ncol(both_participants_df)` variables related to the participants' baseline demographics and amyloid $\beta$ 42/40 ratio measured every two years from the baseline to the eighth year. Among the `r length(unique(both_participants_df$id))` participants, `r sum(both_participants_df$sex == "Female") / 5` are female, `r sum(both_participants_df$apoe4 == "APOE4 carrier") / 5` are APOE4 carriers. The average current age at baseline is `r round(mean(both_participants_df$current_age), 2)`, and the average length of education at baseline is `r round(mean(both_participants_df$education), 2)` years.